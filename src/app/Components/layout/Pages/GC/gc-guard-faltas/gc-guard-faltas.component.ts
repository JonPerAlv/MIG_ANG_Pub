...
import { ModalGCGuardFaltasComponent } from '../Modales/modal-gcguard-faltas/modal-gcguard-faltas.component';
...
@Component({
  selector: 'app-gc-guard-faltas',
  templateUrl: './gc-guard-faltas.component.html',
  styleUrls: ['./gc-guard-faltas.component.css'],
  providers: [MatFormFieldModule, MatInputModule, MatTableModule, MatSortModule, MatPaginatorModule, MatGridListModule],
})
export class GCGuardFaltasComponent implements AfterViewInit {
  displayedColumns: string[] = ['cCURP', 'cPrimerApellido', 'cSegundoApellido', 'cNombre', 'cDescMotivoFalta', 'cHospital', 'cObservacion', 'Registrar', 'Editar', 'Eliminar'];
  MidataSource: MatTableDataSource<VGCFaltasDTO>;
  ...
  obtenerFaltas() {
    ...
    let request = this.formularioBusqueda.value as VGCFaltasDTO;
    ...
    if (this.formularioBusqueda.value.iCveTipoPersona === 1 ||
      ...
      if (
        ...
        if (this.formularioBusqueda.value.iCveTipoPersona === 1) {
          this._gCService.leerListaNinosFaltas(request).subscribe({
            next: (data) => {
              if (data.status)
                if (!data.value) {
          ...
          if (this.formularioBusqueda.value.iCveTipoPersona === 2) {
            this._gCService.leerListaPersonalFaltas(request).subscribe({
              next: (data) => {
                if (data.status)
  ...
  eliminarFalta(registro: VGCFaltasDTO) {
    this.mostrarLoading = true;
    ...
    Swal.fire({
      ...
      if (resultado.isConfirmed) {
        ...
        if (_iCveRegistroEvento > 0) {
          ...
          this._gCService.eliminarFalta(_iCveRegistroEvento).subscribe({
            next: (data) => {
